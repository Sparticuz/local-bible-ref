...
test('a range allows a verse absent between its endpoints', async () => {
	const actsEight = Array.from({ length: 15 }, (_, index) => index + 26)
		.filter((verse) => verse !== 37)
		.map((verse) => `<sup>${verse}</sup> Acts verse ${verse}.`)
		.join('\n');
	const passageSuggest = createPassageSuggest(actsEight);
	...
	assert.equal(suggestions.length, 1);
	assert.equal(suggestions[0].text, `${actsEight}\n\n`);
});
